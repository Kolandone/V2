...
# Prompt user to select between "hamrah" (1) and "irancell" (2)
...
if [ -z "$user_input_domain_port" ]; then
    ...
    fetched_domain_port=$(echo "$user_choice" | bash <(curl -fsSL https://raw.githubusercontent.com/Kolandone/V2/main/installr.sh) | grep -oP '(\d{1,3}\.){3}\d{1,3}:\d+' | head -n 1)
...
# Run the installr.sh script and pipe in the selected option when prompted
output=$({ echo "$user_choice"; echo "wire-g"; } | bash <(curl -fsSL https://raw.githubusercontent.com/Kolandone/V2/main/installr.sh))
