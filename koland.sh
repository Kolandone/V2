...
echo -e "\n${CYAN}Uploading the config file to bashupload.com...${RESET}"
upload_link=$(curl https://bashupload.com/ -T "$output_file" 2>/dev/null)
if [ -n "$upload_link" ]; then
    echo -e "${GREEN}File uploaded successfully!${RESET}"
    echo -e "${CYAN}Share this link with the user:${RESET}"
    echo -e "${YELLOW}$upload_link${RESET}"
    echo -e "${CYAN}Note: The file will be available for 3 days and can be downloaded only once.${RESET}"
else
    echo -e "${RED}Upload failed. Please upload the file manually to a service like Google Drive or Dropbox.${RESET}"
    echo -e "${CYAN}File saved as: $output_file${RESET}"
fi
